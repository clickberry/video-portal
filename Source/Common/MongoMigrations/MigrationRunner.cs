...
namespace MongoMigrations
{
    ...
    public class MigrationRunner
    {
        ...
        {
            var serverAddress = string.IsNullOrEmpty(Database.Server.ReplicaSetName)
                ? Database.Server.Instance.Address
                : Database.Server.Instances[0].Address;
            return string.Format("Updating server {0} database {1}", serverAddress, Database.Name);
        }
